...
Validation
----------
The stakeholders, the ones that make use of Elasticsearch, developers themselves, are who have chosen it as their search solution over the competition, which includes projects such as [Solr](http://lucene.apache.org/solr/), via acceptance testing, that is, each developer assesses the system and its features against their particular use case. 
